...
class Float8InferenceRecipe(Enum):
    """FP8 recipes for inference quantization of MoE expert weights."""
...
@register_as_pytree_constant
@dataclass
class Float8DynamicActivationFloat8WeightMoEConfig(InferenceOpBaseConfig):
    """
    Configuration for Float8 dynamic activation + static weight quantization
    of MoE expert weights.
    ...
    def from_recipe(
        cls,
        recipe: Float8InferenceRecipe,
    ) -> "Float8DynamicActivationFloat8WeightMoEConfig":
        """Factory method to create config from a recipe."""
        if recipe == Float8InferenceRecipe.FP8_ROWWISE:
            return cls()
        else:
            raise ValueError(f"Unsupported FP8 inference recipe: {recipe}")

    def __eq__(self, other):
        if isinstance(other, Float8DynamicActivationFloat8WeightMoEConfig):
            return (
...
@register_quantize_module_handler(Float8DynamicActivationFloat8WeightMoEConfig)
def _moe_inference_transform(
    module: nn.Module,
    config: Float8DynamicActivationFloat8WeightMoEConfig,
    parameter_name: Optional[str] = None,
) -> nn.Module:
    """
    Swaps `torch.nn.Parameter` data tensor with Float8InferenceWeightWrapperTensor
    for FP8 inference quantization of MoE expert weights.
    """
