...
4. quantize_() with Float8DynamicActivationFloat8WeightMoEConfig (skips 2D bias automatically)
...
def run_test(spec: ModelSpec) -> Dict[str, Any]:
    ...
    try:
        ...
        quantize_(model_q, Float8DynamicActivationFloat8WeightMoEConfig(), filter_fn=filter_fn)

        # Verify quantization: 3D → WOQ, 2D → BF16
        for name, mod in find_experts_modules(model_q):
            for pname, p in mod.named_parameters(recurse=False):
                is_woq = isinstance(p.data, Float8InferenceWeightWrapperTensor)
                if p.ndim == 3:
                    if is_woq:
